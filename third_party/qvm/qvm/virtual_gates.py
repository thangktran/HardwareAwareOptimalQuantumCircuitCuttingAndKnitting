...
class VirtualBinaryGate(Barrier, abc.ABC):
    def __init__(self, original_gate: Gate, label=""):
        self._original_gate = original_gate
        super().__init__(num_qubits=original_gate.num_qubits, label=label if len(label)!=0 else f"v_{original_gate.name}")
...
class VirtualMove(VirtualBinaryGate):
    def __init__(self, originalGate):
        super().__init__(originalGate, label=f"VirtualMove {originalGate.label}")
